...
export function generateSitemapXML(pageContexts) {
  try {
    ...
    // Copiar arquivos estáticos para o build (CRÍTICO para produção)
    copyRobotsTxtToBuild(distClientPath)
    copyAdsTxtToBuild(distClientPath)
    ...
    console.log('✅ Arquivos estáticos copiados para build!')
...
function copyAdsTxtToBuild(distClientPath) {
  try {
    const sourceAdsPath = path.join(process.cwd(), 'ads.txt')
    const destAdsPath = path.join(distClientPath, 'ads.txt')

    // Verificar se o arquivo source existe
    if (fs.existsSync(sourceAdsPath)) {
      // Copiar ads.txt para o build
      fs.copyFileSync(sourceAdsPath, destAdsPath)
      console.log('📋 Ads.txt copiado com sucesso para o build!')
    } else {
      console.warn('⚠️ Arquivo ads.txt não encontrado na raiz do projeto')
    }
  } catch (error) {
    console.error('❌ Erro ao copiar ads.txt:', error)
  }
}
